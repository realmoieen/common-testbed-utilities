...
import de.bsi.testbedutils.cvc.cvcertificate.exception.CVAuthorityRefNotValidException;
import de.bsi.testbedutils.cvc.cvcertificate.exception.CVBufferNotEmptyException;
import de.bsi.testbedutils.cvc.cvcertificate.exception.CVDecodeErrorException;
import de.bsi.testbedutils.cvc.cvcertificate.exception.CVInvalidDateException;
import de.bsi.testbedutils.cvc.cvcertificate.exception.CVInvalidECPointLengthException;
import de.bsi.testbedutils.cvc.cvcertificate.exception.CVInvalidKeySourceException;
import de.bsi.testbedutils.cvc.cvcertificate.exception.CVInvalidOidException;
...
import de.bsi.testbedutils.cvc.cvcertificate.exception.CVUnknownCryptoProviderException;
...
//						m_rLog << "0x65 Certificate extension tag not found" << std::endl;
	...
	public boolean checkSign(CVCertificate authCert, CVCertificate cvRootCert) throws CVInvalidKeySourceException, CVMissingKeyException, InvalidKeyException, NoSuchAlgorithmException, NoSuchProviderException, SignatureException, InvalidKeySpecException, CVUnknownAlgorithmException, CVUnknownCryptoProviderException, CVKeyTypeNotSupportedException{
		...
		if (
				cvRootCert.getPublicKey().getAlgorithm().compareTo(TAAlgorithm.ECDSA_SHA_512) == 0 ||
	...
	protected DataBuffer generateBody() throws CVAuthorityRefNotValidException, CVInvalidOidException, CVSignOpKeyMismatchException, CVInvalidKeySourceException, CVMissingKeyException, CVKeyTypeNotSupportedException{
		...
		if(m_certAuthRef.length() > 0 && m_certAuthRef.length() <= 21)
		{
		...
		if(m_certHolderRef.length() > 0 && m_certHolderRef.length() <= 21)
		{
