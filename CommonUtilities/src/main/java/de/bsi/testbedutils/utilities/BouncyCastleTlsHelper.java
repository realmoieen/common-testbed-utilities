...
public class BouncyCastleTlsHelper {
    ...
    public static int convertNamedCurveStringToInt(String namedCurve) throws IllegalArgumentException {
        return convertBCEnumerationStringToInt(NamedGroup.class, namedCurve);
    }
    ...
    public static String convertNamedCurveIntToString(int namedCurve) {
        return convertBCEnumerationIntToString(NamedGroup.class, namedCurve);
    }
    ...
    public static LinkedList<String> getAllNamedCurveStrings() {
        return getAllBCEnumerationStrings(NamedGroup.class, int.class);
    }
     ...
     * There are no magic values to indicate a failed conversion, so an IllegalArgumentException is thrown instead.   
     * @param signatureAlgorithm TLS signature algorithm given as string
     ...
     * @param signatureAlgorithm TLS signature algorithm given as integer value found in class SignatureAlgorithm
     * @return corresponding string or null, if conversion fails
     */
    ...
    private static <T> T convertBCEnumerationStringToType(@SuppressWarnings("rawtypes") Class enumClazz, Class<T> typeClazz, String aString) throws IllegalArgumentException {
        // search for public final static <typeClazz> fields in class enumClazz, which should basically be an enumeration.
        ...
        throw new IllegalArgumentException("Could not convert string '" + aString + "' to a corresponding object from class " + enumClazz.getCanonicalName());
    }

    private static LinkedList<String> getAllBCEnumerationStrings(@SuppressWarnings("rawtypes") Class enumClazz, @SuppressWarnings("rawtypes") Class typeClazz) {
        // search for public final static <typeClazz> fields in class enumClazz, which should basically be an enumeration.
    ...
    public static org.bouncycastle.tls.SignatureAndHashAlgorithm convertSignatureAndHashAlgorithmStringToClassOrig(String signatureAndHashAlgorithm) {
        if (signatureAndHashAlgorithm != null && !signatureAndHashAlgorithm.isEmpty()) {
            Pattern pattern = Pattern.compile("(.*)with(.*)");
            Matcher matcher = pattern.matcher(signatureAndHashAlgorithm.toLowerCase(Locale.ROOT));
            if (matcher.find()) {
                ...
                String extractedSignature = matcher.group(2);
                short signature = convertBCEnumerationStringToShort(SignatureAlgorithm.class, extractedSignature);
                return new org.bouncycastle.tls.SignatureAndHashAlgorithm(hash, signature);
            }
        }
    ...
    /**
     * Converts a TLS protocol version given as string to a corresponding object found in class ProtocolVersion.
     * There are no magic values to indicate a failed conversion, so an IllegalArgumentException is thrown instead.   
     * @param protocolVersion TLS protocol version given as string
     * @return corresponding object found in class ProtocolVersion
     * @throws IllegalArgumentException if conversion fails
     */
    public static ProtocolVersion convertProtocolVersionStringToObject(String protocolVersion) throws IllegalArgumentException {
        return convertBCEnumerationStringToType(ProtocolVersion.class, ProtocolVersion.class, protocolVersion);
    }
    ...
    public static Certificate convertCertificateChainFromJavaToBC(X509Certificate[] certificateChain) throws CertificateEncodingException {
        TlsCertificate[] tempList = new TlsCertificate[certificateChain.length];
        for(int i=0 ; i < certificateChain.length ; i++) {
            X509CertificateHolder sigCert = new JcaX509CertificateHolder(certificateChain[i]);
            tempList[i] = new BcTlsCertificate(new BcTlsCrypto(new SecureRandom()), sigCert.toASN1Structure());
        }
